...
const CompletedTodos: React.FC = () => {
  ...
  useEffect(() => {
    if (data?.data) {
      const completedTodos = data.data.filter((todo) => todo.isCompleted);
      const savedOrder = localStorage.getItem("completedTodoOrder");
      let orderedTodos: Todo[] = [];

      if (savedOrder) {
        const order = JSON.parse(savedOrder);
        const ordered = order
          .map((id: string) => completedTodos.find((todo) => todo.id === id))
          .filter(Boolean) as Todo[];

        const unorderedTodos = completedTodos.filter(
          (todo) => !order.includes(todo.id)
        );

        orderedTodos = [...unorderedTodos.reverse(), ...ordered];
      } else {
        orderedTodos = completedTodos.slice().reverse();
      }
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    localStorage.setItem("completedTodoOrder", JSON.stringify(todoOrder));
  };
